...
Receptor preparation was tested across 50 sampled public PDB structures. The original cohort passed 10/10. A second reproducible cohort used the current RCSB entry-ID holdings list, excluded the original ten, and selected 40 downloadable legacy-format PDB entries with random seed `20260819`; 36/40 produced prepared receptors and completed pocket preparation. Of those 36, 12 passed strict Meeko, 19 passed conservative PDBFixer repair followed by strict Meeko, and 5 passed the documented final Meeko batch cleanup. Combined performance was therefore 46/50.

The four second-cohort stops required scientific input rather than another generic automatic cleanup: a heme/cofactor CCD-template parsing failure, protein-DNA alternate-location/template conflicts, linked glycan templates, and ambiguous histidine protonation. Docking Universal did not silently delete these components or guess the histidine state to increase the pass count. This is a software-path stress test rather than biological validation. PDBFixer repair is limited to alternate-location resolution, recognized nonstandard-residue mappings, and missing side-chain heavy atoms; it does not construct missing loops or terminal atoms, and every invocation retains `pdbfixer_audit.json` for review.
