...
Every automatically generated PDF ends with a reproducibility section that assigns each reported quantity or visual to the program that produced it and provides primary references. Every control-backed new run includes a full comparison table for Docking Universal, Python, RDKit, MolScrub, Meeko, and AutoDock Vina. Matching reports state `Checked: no software version differences detected`; differences or unavailable versions are identified row by row. Additional analysis/reporting tools are listed separately. The same information is written to `report/software_versions_and_references.json`.
