...
In an interactive run, omitting `--protocol` opens a guided choice: select `.duprotocol` or `protocol.json` with Finder, select a completed control/study folder and discover its approved protocol, or enter a path manually. A bundle is extracted safely, every retained file hash is verified, and the protocol is accepted only if its sampling, ranking, and independent-seed criteria passed. The receptor and box hashes must still match. Inputs can be one SDF, a multi-record SDF, or a directory of SDF files.
...
Every automatically generated PDF ends with a reproducibility section that assigns each reported quantity or visual to the program that produced it and provides primary references. The section records whether receptor conversion succeeded with strict Meeko, used conservative PDBFixer repair before retrying strict Meeko, or required the final documented Meeko batch-cleanup fallback. PDBFixer is not described as having modified a receptor unless retained preparation artifacts show that it ran. Every control-backed new run includes a full comparison table for Docking Universal, Python, RDKit, MolScrub, Meeko, PDBFixer, and AutoDock Vina. Matching reports state `Checked: no software version differences detected`; differences or unavailable versions are identified row by row. Additional analysis/reporting tools are listed separately. The same information, the preparation path, and the PDBFixer audit location when applicable are written to `report/software_versions_and_references.json`.
