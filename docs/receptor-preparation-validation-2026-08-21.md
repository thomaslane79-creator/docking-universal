...
The tested order is strict Meeko, conservative PDBFixer followed by strict Meeko, documented disulfide/histidine handling where applicable, then a narrow ADFRsuite fallback only after Meeko reports linked deposited chemistry. The ADFRsuite route preserves directly `LINK`ed deposited components in the input path. Automatic unmatched-component deletion is not used. If all safe routes fail, an interactive user may explicitly request Meeko's `--allow_bad_res` attempt; its decision and log are retained. It is not a safe chemistry repair and never creates approval by itself.
