...
The evidence is contained in this record's complete 100-PDB manifests below, its route/outcome tables, and the reproducible repository checks: [CLI/integration checks](../tests/test_cli.sh), [PDBFixer tests](../tests/test_pdbfixer_preclean.py), [CCD audit tests](../tests/test_ccd_audit.py), and [report tests](../tests/test_report_cavity.py). Compact maintained fixtures live under `tests/inputs/` and `tests/expected_outputs/`; complete public-structure studies are generated locally. The listed PDB IDs identify the public coordinate inputs, which are not redistributed. The separate [main validation index](validation.md) records installed-command, CI, and end-to-end workflow evidence.
...
A fresh 100-entry report-generation stress run initially completed 79 full PDF/protocol/bundle outputs and stopped during receptor preparation for 21 entries. The original 79 include `5NBX`, which completed after the documented guided histidine-template selection. Review identified a generic filtering regression: some coordination waters and unsupported single-atom ions in deposited `LINK` records were being retained as though they were covalent components. After correcting that distinction, complete v0.6.4 protocol reports were generated for all six examined affected receptors: `1GYN` through PDBFixer followed by strict Meeko; `5K8R`, `3ET8`, `1P5S`, and `4ER8` through strict Meeko; and `2NSY` through the narrow ADFRsuite fallback. These focused checks bring the demonstrated automatic report paths to 85 distinct structures, but they are not a single clean rerun of the complete panel.

A separate interactive 5KRH test exercised the final model-changing route. The workflow stopped, required explicit approval, completed after approval, and recorded an exact inventory of 31 removed standard amino-acid residues. These correspond to incomplete or template-mismatched protein/peptide residues rather than a collection of optional solvent molecules or post-translational modifications. The generated [5KRH site-guided protocol report](assets/5KRH-user-approved-removal-cavity-report.pdf) labels this a high-severity structural modification; the same warning is carried into later reports that reuse the protocol, while the exact TSV inventory and raw log remain in the retained artifacts and bundle. Including this explicitly modified receptor yields complete retained reports for 86 of the 100 distinct structures. This verifies audit and reporting behavior, not the biological suitability of the altered receptor.

The [downloadable 86-report validation-evidence bundle](https://github.com/thomaslane79-creator/docking-universal/releases/download/v0.6.4/docking-universal-100-pdb-validation-evidence-86-reports.zip) contains the original summaries and reports, the six complete focused v0.6.4 regression reports, the 5KRH report, and a supplemental route index. Its SHA-256 digest is `add320f3868937bda8a3983ce78a07e07369b2a211abe5432a4d218bed78c317`. Fourteen structures remain without complete reports, and a single clean v0.6.4 rerun of all 100 remains pending.
